"""Inference-only TopKNonlinearSAE variant, similar in spirit to StandardSAE but using a TopK-based activation."""
...
class PWLActivation(nn.Module):
    def __init__(
        self, n_features: int, n_knots: int = 16, grid_range: tuple[int, int] = (-5, 5)
    ):
        super().__init__()
        self.n_knots = n_knots

        knots = torch.linspace(grid_range[0], grid_range[1], n_knots)
        self.register_buffer("knots", knots)
        self.spacing = (grid_range[1] - grid_range[0]) / (n_knots - 1)

        # (n_features, n_knots) — initialized as ReLU
        self.values = nn.Parameter(
            torch.relu(knots).unsqueeze(0).expand(n_features, -1).clone()
        )

    def forward(self, z: torch.Tensor) -> torch.Tensor:
        """z: (batch, n_features) → (batch, n_features)"""
        orig_shape = z.shape
        z = z.reshape(-1, orig_shape[-1])

        z_type = z.dtype

        batch, n_feat = z.shape

        z_c = z.clamp(self.knots[0], self.knots[-1] - 1e-6)

        # Segment index: (batch, n_features)
        idx = ((z_c - self.knots[0]) / self.spacing).long()
        idx = idx.clamp(0, self.n_knots - 2)

        # Expand values for batch dimension and gather
        # values: (n_features, n_knots) → (batch, n_features, n_knots)
        vals = self.values.unsqueeze(0).expand(batch, -1, -1)

        # idx for gather: (batch, n_features, 1)
        idx_g = idx.unsqueeze(-1)

        left = vals.gather(2, idx_g).squeeze(-1)  # (batch, n_features)
        right = vals.gather(2, idx_g + 1).squeeze(-1)  # (batch, n_features)

        # Interpolation weight
        t = (z_c - self.knots[0] - idx.float() * self.spacing) / self.spacing

        out = left + t * (right - left)
        out = out.to(z_type)
        return out.reshape(orig_shape)

    def relu_deviation(self) -> torch.Tensor:
        """
        Per-feature L2 distance from ReLU, normalized by ReLU norm.
        Returns: (n_features,) values in [0, ∞), where 0 = exactly ReLU.
        """
        with torch.no_grad():
            relu_vals = torch.relu(self.knots).unsqueeze(0)  # (1, n_knots)
            diff = self.values - relu_vals  # (n_features, n_knots)

            deviation = diff.pow(2).sum(dim=-1).sqrt()  # (n_features,)
            relu_norm = relu_vals.pow(2).sum(dim=-1).sqrt()  # (1,)

            return deviation / (relu_norm + 1e-8)  # (n_features,)
...
class PWLTopK(nn.Module):
    def __init__(
        self,
        n_features: int,
        k: int,
        n_knots: int = 16,
        grid_range: tuple[int, int] = (-5, 5),
        use_sparse_activations: bool = False,
    ):
        super().__init__()
        self.pwl = PWLActivation(n_features, n_knots, grid_range)

        self.topk = TopK(k, use_sparse_activations)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.topk(x)  # self.pwl(x) # Try jumpgelu


@dataclass
class TopKNonlinearSAEConfig(SAEConfig):
    """
    Configuration class for TopKNonlinearSAE inference.
...
class TopKNonlinearSAE(SAE[TopKNonlinearSAEConfig]):
    """
    ...
    def __init__(self, cfg: TopKNonlinearSAEConfig, use_error_term: bool = False):
        """
        Args:
    ...
    def fold_W_dec_norm(self) -> None:
        if not self.cfg.rescale_acts_by_decoder_norm:
            raise NotImplementedError(
                "Folding W_dec_norm is not safe for TopKNonlinearSAEs when rescale_acts_by_decoder_norm is False, as this may change the topk activations"
            )
        _fold_norm_topk(W_dec=self.W_dec, b_enc=self.b_enc, W_enc=self.W_enc)


@dataclass
class TopKNonlinearTrainingSAEConfig(TrainingSAEConfig):
    """
    Configuration class for training a TopKNonlinearTrainingSAE.
...
class TopKNonlinearTrainingSAE(TrainingSAE[TopKNonlinearTrainingSAEConfig]):
    """
    ...
    def __init__(
        self, cfg: TopKNonlinearTrainingSAEConfig, use_error_term: bool = False
    ):
        super().__init__(cfg, use_error_term)
        ...
        self.activation_fn = PWLTopK(
            ...
            n_knots=10,
            grid_range=(-5, 5),
            use_sparse_activations=cfg.use_sparse_activations,
        )
    ...
    @override
    def training_forward_pass(self, step_input: TrainStepInput):
        output = super().training_forward_pass(step_input)
        l2_from_relu = self.activation_fn.pwl.relu_deviation()
        output.metrics["mean_l2_from_relu"] = l2_from_relu.mean()
        output.metrics["90th_l2_from_relu"] = l2_from_relu.quantile(0.90)
        output.metrics["99th_l2_from_relu"] = l2_from_relu.quantile(0.99)
        return output
    ...
    @override
    def get_activation_fn(self) -> Callable[[torch.Tensor], torch.Tensor]:
        return TopK(69)  # dummy, set to self.pwl_act
...
def _init_weights_topk(
    sae: SAE[TopKNonlinearSAEConfig] | TrainingSAE[TopKNonlinearTrainingSAEConfig],
) -> None:
