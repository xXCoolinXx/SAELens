...
if __name__ == "__main__":
    ...
    for hook_layer in MODEL_CONFIGS[model_name]["layers"]:  # type: ignore
        ...
        selected_saes = [(f"{model_name}_layer_{hook_layer}_mixae_sae_l0_192_8k", sae)]

        context_idx = torch.arange(0, 1, device=device)  # type: ignore
        token_idx = torch.arange(0, 1, device=device)  # type: ignore
        all_idx = torch.arange(0, 1, device=device)  # type: ignore
